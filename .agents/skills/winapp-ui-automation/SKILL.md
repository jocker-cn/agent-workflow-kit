...
version: 0.5.0
---
## Repository-local invocation

This project vendors the Microsoft `v0.5.0` skill together with `@microsoft/winappcli@0.5.0`.
Do not use or install a global `winapp` binary. In every command below, invoke the same command
through the project script:

```text
winapp ui <arguments>  ->  pnpm desktop <arguments>
```

For example, `winapp ui inspect -a notepad --interactive` becomes
